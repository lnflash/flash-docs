---
title: Flash Partners
description: A short overview of the technology partners that have helped Flash evolve.
---

Flash has worked with a number of technology partners over time. Some of those integrations are now legacy, and some remain part of the current stack.

### Current or recent building blocks

- **Breez SDK** — helps power the Lightning wallet experience
- **IBEX Mercado** — helps power the USD cash wallet and payment infrastructure

### Legacy / historical integrations

- **Galoy Project**
- **BTCPay Server**

## What users should know

If you are using Flash today, the important parts are the app pages and wallet guides:

- [Cash Wallet](/guides/cash-wallet/)
- [Bitcoin Wallet](/guides/bitcoin-wallet/)
- [Swap Between Wallets](/guides/swap/)
- [Top Up](/guides/top-up/)
- [Cash Out](/guides/cash-out/)

If you need help with the current product, contact Flash support.
